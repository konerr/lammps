...
FixEMDPDSource::FixEMDPDSource(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (strcmp(style,"emdpd/source") != 0 && narg < 4)
    error->all(FLERR,"Illegal fix emdpd/source command");
  ...
  if (option == 0) {
    if (narg != 9 ) error->all(FLERR,"Illegal fix emdpd/source command (5 args for sphere)");
...
void FixEMDPDSource::init()
{
}

/* ---------------------------------------------------------------------- */

void FixEMDPDSource::post_force(int /*vflag*/)
{
  double **x = atom->x;

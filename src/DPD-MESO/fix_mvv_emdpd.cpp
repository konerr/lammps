...
/* ----------------------------------------------------------------------
   ...
   v and emdpd_T) using the modified velocity-Verlet (MVV) algorithm.
...
FixMvvEMDPD::FixMvvEMDPD(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (strcmp(style,"mvv/emdpd") != 0 && narg < 3)
    error->all(FLERR,"Illegal fix mvv/emdpd command");
...
void FixMvvEMDPD::initial_integrate(int /*vflag*/)
{
  double dtfm,dtT;
  ...
  if (mask[i] & groupbit) {
     ...
     emdpd_temp[i] += 2.0 * verlet * dtT * emdpd_flux[i];
  }
}

/* ---------------------------------------------------------------------- */

void FixMvvEMDPD::final_integrate()
{
  double dtfm, dtT;

  // update v and emdpd_temp of atoms in group
  ...
  if (mask[i] & groupbit) {
     ...
     emdpd_temp[i] = vest_temp[i] + dtT * emdpd_flux[i];
  }
}

/* ---------------------------------------------------------------------- */

void FixMvvEMDPD::reset_dt()
{
  dtv = update->dt;
  dtf = 0.5 * update->dt * force->ftm2v;
}
